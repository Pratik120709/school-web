    ...
    <div class="col-12">
        <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form class="custom-validation" action="{{ route('admin.department.storeDepartment') }}"
                                ...
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        <button class="btn btn-info waves-effect waves-light btn-sm"
                                            onclick="editDepartment(this)" data-id="{{$data->id}}"
                                            data-program_id="{{ $data->program_id}}" data-name="{{ $data->name}}"
...
function editDepartment(obj) {
    ...
    let program_id = $(obj).attr('data-program_id');
    let name = $(obj).attr('data-name');
    let status = $(obj).attr('data-status');

    $("#department input[name='name']").val(name).trigger('change');
    $("#department select[name='program_id']").val(program_id).trigger('change');
    $("#department select[name='status']").val(status).trigger('change');
    $("#department button[type='submit']").text('Update');

    var url = "{{ url('admin/department/updateDepartment') }}/" + id;
    $('#department').attr('action', url);

    $('html,body').animate({
        scrollTop: $("#department").offset().top - 100
    }, 'fast');
}
$('#reset-form').on('click', function() {
    ...
    // Reset the select2 dropdown values
    $('#department .select2').val('').trigger('change');

    // Reset the input values
    $('#department input[name="name"]').val('');
    $('#department select[name="status"]').val('').trigger('change');

    // Reset Parsley validation
    $('#department').parsley().reset();

    // Change button text back to 'Submit'
    $("#department button[type='submit']").text('Submit');
});
